...
const productsDisplay = (arrayOfProducts) => {
    const productsArray = arrayOfProducts.map(product => `
    <div class="product">
     ...
     `).join("")
    productsWrapper.innerHTML = productsArray;
}
...
searchInput.addEventListener("keyup", (event) => {
    ...
    if (value) {
        const filterArray = data.filter(item => item.name.toLocaleLowerCase().includes(value))

        productsDisplay(filterArray);
...
const setCategories = () => {
    ...
    const filterCategories = allCategories.filter((item, index) =>
        allCategories.indexOf(item) === index
    );

    categories.innerHTML = filterCategories.map(category => `
  ...
  `).join("");
    ...
    categories.addEventListener("click", (event) => {
        ...
        if (selectedCategory) {
            ...
            productsDisplay(sortProductsByCategory);
        } else {
            productsDisplay(data);
        }
    });
